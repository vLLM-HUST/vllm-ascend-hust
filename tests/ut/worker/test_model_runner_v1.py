...
class TestNPUModelRunnerKVCache(unittest.TestCase):
    ...
    def test_initialize_routes_v41_before_standardized_allocator(self):
        from tests.deepseek_v41_utils import make_cache_config
        ...
        runner.shared_kv_cache_layers = {}
        runner.model_config.hf_text_config.model_type = "deepseek_v41"
        runner.kv_caches = []
        config = make_cache_config(4)
        raw = {"raw": torch.empty(1)}
        caches = {
            "model.layers.1.self_attn.long_kv_cache": torch.empty(1),
            "model.layers.0.self_attn.swa_cache": torch.empty(1),
        }
        layers = {name: V41CacheLayer() for name in caches}
        ...
        with (
            ...
            patch("vllm_ascend.worker.model_runner_v1.allocate_kv_cache") as allocate_standard,
            patch(
                "vllm_ascend.worker.model_runner_v1.v41_cache_layer_type",
                V41CacheLayer,
            ),
            patch("vllm.v1.worker.utils.bind_kv_cache") as bind_standard,
        ):
            result = runner.initialize_kv_cache_tensors(config)

        self.assertIs(result, caches)
        allocate_v41.assert_called_once_with(config)
        reshape_v41.assert_called_once_with(config, raw)
        allocate_standard.assert_not_called()
        bind_standard.assert_not_called()
        self.assertEqual(
            runner.kv_caches,
            [caches[name] for name in sorted(caches)],
        )
        for name, layer in layers.items():
            self.assertEqual(layer.kv_cache, [caches[name]])
    ...
    def test_dsv4_main_materializes_real_planner_geometry_once(
        ...
        """Verify MRV1 materializes the shared DSV4 planner geometry."""
...
class TestNPUModelRunnerOutputTokenIds(unittest.TestCase):
    ...
    # This checks token metadata on CPU; pinned allocation is covered separately.
